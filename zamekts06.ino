#include "zamek-globals.h" // Include the global header file, which likely contains shared constants, definitions, and includes.
...
void setup() {
    ...
    zamekDisplay.wipeScreen(); // Custom function to clear the display buffer.
    display.drawBitmap(0, 0, logo, 128, 32, WHITE); // Draw a logo bitmap (128x32) at the top-left corner.
    zamekDisplay.zamekdisplay(); // Push the display buffer to the screen.
    ...
    zamekDisplay.drawScreen(); // Update the screen to reflect the current state.
...
void loop() {
    ...
    zamekIo.handleButtonChecker(); // Check the trackball and button for state changes.

    zamekDisplay.drawScreen(); // Update the screen to show the latest state.
}
